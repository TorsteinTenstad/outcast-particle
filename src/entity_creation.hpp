...
EntityHandle CreateText(ECSScene& level, sf::Vector2f position, std::string content, unsigned int text_size = 120, std::optional<sf::Vector2f> layout_size = std::nullopt, TextOrigin text_origin = TextOrigin::REFERENCE_HEIGHT_CENTER);
...
EntitiesHandle CreateCanDisableButtonWithIcon(ECSScene& level, sf::Vector2f position, sf::Vector2f size, std::function<void(void)> button_function, std::string icon_path, std::string tooltip_text, std::function<bool(void)> deactivate_function, sf::Keyboard::Key shortcut_key, bool shortcut_requires_ctrl = false);
