...
std::vector<int> AddKeyConfigButton(Level& level, sf::Keyboard::Key* key, sf::Vector2f position, int button_text_id);
std::vector<int> AddOptionsButton(Level& level, std::function<void(void)> on_click, sf::Vector2f position, int button_text_id);
