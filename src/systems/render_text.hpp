...
private:
	...
	std::map<std::string, sf::Text> sample_text_;
	...
	void Update(CursorAndKeys& cursor_and_keys, Level& level, float dt)
	{
		...
		for (auto const& [entity_id, entity_text] : text_map)
		{
			...
			if (font_.count(entity_text.font_path) == 0)
			{
				...
				sample_text_[entity_text.font_path].setString("N");
				sample_text_[entity_text.font_path].setFont(font_[entity_text.font_path]);
			}
			if (text_height_.count(entity_text.size) == 0)
			{
				sample_text_[entity_text.font_path].setCharacterSize(entity_text.size);
				text_height_[entity_text.size] = sample_text_[entity_text.font_path].getLocalBounds().height / 2 + sample_text_[entity_text.font_path].getLocalBounds().top;
			}
