...
void SetTextures(Level& level)
{
	for (auto const& [entity, player, player_behaviors, charge, draw_info, children, face] : level.GetEntitiesWith<Player, PlayerBehaviors, Charge, DrawInfo, Children, Face>())
	{
		...
		else
		{
			...
			//int category = FindClosest(PARTICLE_CHARGE_CATEGORIES, nonzero_charge);
			//draw_info->image_path = PLAYER_PARTICLE_NEUTRAL_TEXTURES[category];F
		}
		face->image_path = (TEXTURES_DIR / "face_").string() + std::string((nonzero_charge > 0) ? "plus.png" : "minus.png");
	}
}
