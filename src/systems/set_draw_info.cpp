...
static Entity CreateStrengthIndicator(Level& level)
{
	...
	level.AddComponent<Shader>(entity)->fragment_shader_path = (SHADERS_DIR / "strength_indicator.frag").string();
...
{
	EntityHandle handle = CreateTexturedRectangle(level, sf::Vector2f(0, 0), sf::Vector2f(1, 1) * 0.5f * float(BLOCK_SIZE), 0, (TEXTURES_DIR / "lock.png").string(), false);
	Entity entity = GetEntity(handle);
...
static Entity CreateVelocityIndicator(Level& level)
{
	...
	level.AddComponent<Shader>(entity)->fragment_shader_path = (SHADERS_DIR / "velocity_indicator.frag").string();
	return entity;
}
