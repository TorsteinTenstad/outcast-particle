...
{
	for (auto [entity, animated_property, position] : level.GetEntitiesWith<AnimatedPosition, Position>())
	{
		position->position = animated_property->animation_func(globals.time - animated_property->start_time);
	}
	for (auto [entity, animated_property, fill_color] : level.GetEntitiesWith<AnimatedOpacity, FillColor>())
	{
		fill_color->color.a = animated_property->animation_func(globals.time - animated_property->start_time);
	}
	for (auto [entity, animated_property, text] : level.GetEntitiesWith<AnimatedOpacity, Text>())
	{
