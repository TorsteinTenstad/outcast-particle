...
void BackgroundSystem::Update(Level& level, float dt)
{
	...
	for (auto [backgrounds_id, background, draw_info, draw_priority, width_and_height, position] : backgrounds)
	{
		...
		draw_info->image_path = in_edit_mode ? (TEXTURES_DIR / "grid.png").string() : (TEXTURES_DIR / "background.png").string();
		draw_info->image_path = is_main_background ? draw_info->image_path : (TEXTURES_DIR / "background.png").string();
