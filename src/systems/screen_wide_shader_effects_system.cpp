...
void ScreenWideShaderEffectsSystem::Update(Level& level, float dt)
{
	...
	{
		Entity child = GetSingletonScreenWideFragmentShaderChildId<SegmentedGlowEffect>(level, entity, (SHADERS_DIR / "segmented_glow_effect.frag").string(), draw_priority->draw_priority - 1);
		auto [shader, child_draw_priority] = level.GetComponents<Shader, DrawPriority>(child);
		child_draw_priority->draw_priority = draw_priority->draw_priority - 1;
	...
	{
		Entity child = GetSingletonScreenWideFragmentShaderChildId<TwinkleEffect>(level, entity, (SHADERS_DIR / "twinkle.frag").string(), draw_priority->draw_priority + 1);
		auto [shader, child_draw_priority] = level.GetComponents<Shader, DrawPriority>(child);
		child_draw_priority->draw_priority = draw_priority->draw_priority + 1;
