...
void ScreenWideShaderEffectsSystem::Update(Level& level, float dt)
{
	...
	{
		Shader* shader = GetSingletonScreenWideFragmentShaderChildId<SegmentedGlowEffect>(level, entity, "SHADERS_DIR\\segmented_glow_effect.frag", draw_priority->draw_priority - 1);
	...
	{
		Shader* shader = GetSingletonScreenWideFragmentShaderChildId<TwinkleEffect>(level, entity, "SHADERS_DIR\\twinkle.frag", draw_priority->draw_priority + 1);
