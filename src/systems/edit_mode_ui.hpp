...
const std::array<std::string, 8> EDIT_TOOLS = {
	"BPDeleteIndicator",
	"BPWall",
	"BPBounceWall",
	"BPNoBounceWall",
...
const std::array<bool, 8> EDIT_TOOL_IS_DRAG_TOOL = {
	true,
	true,
	true,
	true,
...
const std::array<std::string, 8> EDIT_TOOLS_ICONS = {
	...
	void Update(Level& level, float dt)
	{
		...
		if (!ui->initialized)
		{
			...
			for (int i = 0; i < 8; ++i)
			{
				...
				EditToolButton* edit_tool_button = level.AddComponent<EditToolButton>(entity_id);
				edit_tool_button->tool = EDIT_TOOLS[i];
				edit_tool_button->is_drag_tool = EDIT_TOOL_IS_DRAG_TOOL[i];
