...
void main()
{
	...
	color = blend(color, gem((c_uv-vec2(1*margin+1*gem_r,0))/gem_size)*vec4(vec3(1), clamp(float(n_collected)-0.f, 0.20f, 1.f)));
	color = blend(color, gem((c_uv-vec2(2*margin+3*gem_r,0))/gem_size)*vec4(vec3(1), clamp(float(n_collected)-1.f, 0.20f, 1.f)));
	color = blend(color, gem((c_uv-vec2(3*margin+5*gem_r,0))/gem_size)*vec4(vec3(1), clamp(float(n_collected)-2.f, 0.20f, 1.f)));
